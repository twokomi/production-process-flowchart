...
const categories = {
  CT: {
    name: 'CT',
    fullName: 'Cold Test',
    description: 'From initial review to plate cutting',
    color: '#3b82f6',
    icon: 'fa-snowflake'
  },
  BT: {
    name: 'BT',
    fullName: 'Blast Test',
    description: 'From cutting to BT release',
    color: '#8b5cf6',
    icon: 'fa-hammer'
  },
  WT: {
    name: 'WT',
    fullName: 'Wash Test',
    description: 'From wash bay to WT release',
    color: '#06b6d4',
    icon: 'fa-water'
  },
  IM: {
    ...
    description: 'From installation to IM release',
    color: '#f59e0b',
    icon: 'fa-tools'
  },
  GT: {
    name: 'GT',
    fullName: 'Gate',
    description: 'From IM release to finish',
    color: '#10b981',
    ...
    html += `
      <div class="category-card" style="border-left: 4px solid ${cat.color}" onclick="selectCategory('${key}')">
        ...
        <div class="category-progress">
          ...
          <div class="progress-text">${cat.completedCount} / ${cat.totalSteps} Completed</div>
        </div>
      </div>
    `;
  });
...
async function selectCategory(categoryKey) {
  ...
  if (categorySteps.length === 0) {
    alert('No steps in this category.');
    return;
  }
...
async function loadCurrentStep() {
  ...
  try {
    ...
    alert('Failed to load step information.');
  }
}
      ...
      <div class="criteria-detail-modal">
        ...
        ${criteria.documents && criteria.documents.length > 0 ? `
          <div class="documents-section-mobile">
            <h4 class="documents-title-mobile">Related Documents</h4>
            ${renderDocumentsList(criteria.documents)}
          </div>
        ` : `
          <div class="no-documents-mobile">
            <i class="fas fa-folder-open mr-2"></i>
            No related documents registered.
          </div>
        `}
      </div>
    `;
...
function getDocumentTypeLabel(type) {
  switch(type) {
    case 'work_instruction': return 'Work Instruction';
    case 'quality_spec': return 'Quality Spec';
...
function answerNo() {
  ...
  alert('불합격 처리되었습니다. 해당 Step를 다시 확인해주세요.');
...
function showCategoryCompleted() {
  ...
  document.getElementById('stepQuestion').innerHTML = `
    <div class="completion-card">
      ...
      <h2 class="completion-title">${cat.fullName} Completed!</h2>
      <p class="completion-message">
        모든 Step를 Completed했습니다.<br>
        Proceed to another category or return to main.
      </p>
